    ...
    for method in method_list
        if method[1:6] == "mlef-n" || 
            method[1:9] == "mlef-ls-n"
                for analysis in analysis_list
                    for stat in stat_list
                        data[method * "_" * analysis * "_" * stat] = Array{Float64}(undef, total_gammas,  ensemble_size)
                    end
                end
        else
            for analysis in analysis_list
                for stat in stat_list
                    data[method * "_" * analysis * "_" * stat ] = Array{Float64}(undef, total_gammas, total_inflation, ensemble_size)
                end
            end
        end
    end
