...
def find_optimal_values(method, data):
    ...
    return [iteration_mean, iteration_std]#, iteration_median]
...
color_map = sns.cubehelix_palette(n_colors=20, rot=1.75)
max_scale = 20.0
min_scale = 1.0
...
for method in methods:
    ...
    for i in range(0,2):
        sns.heatmap(iterations[i], linewidth=0.5, ax=ax_list[i+j], cbar_ax=ax0, vmin=min_scale, vmax=max_scale, cmap=color_map)
    j += 2
...
plt.figtext(.025, .2925, r'Median', horizontalalignment='center', verticalalignment='center', fontsize=22, rotation='90')
