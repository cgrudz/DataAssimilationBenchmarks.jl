...
for bkg_cov in bkg_covs
	...
	file = "bkg-" * bkg_cov * "_L96_state_seed_0123_diff_0.000_sysD_40_obsD_" * lpad(dim, 2, "0") * 
        ...
        if cmp(bkg_cov, "ID") == 0
            stab_rmse_id[dim] = mean(filt_rmse[2000:nanl])
        else
            stab_rmse_clima[dim] = mean(filt_rmse[2000:nanl])
        end
    end
end

plot(dims, stab_rmse_id, label = "ID", title="2000: Stabilized Filter RMSE vs. Cycles", legend_position = :topright, margin=15mm, size=(800,500), dpi = 600)
